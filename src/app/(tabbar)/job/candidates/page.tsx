...
export function Page({ params }: { params: { id: string } }) {
  ...
  const getCandidateLists = () => {
    API.get(`${API_CONSTANT?.JOB}?status=hired`)
      .then((res: any) => {
        setCandidateList(res?.data?.data[0]);
      })
      .catch((error) => {
  ...
  return (
    <div>
      <div className="flex items-center text-2xl font-semibold text-meta-purple-1">
        ...
        <p className="ml-2">Candidates Hired</p>
      </div>
      <div className="mt-5 h-screen">
        <table className="w-full text-left text-sm">
          ...
          {candidateList?.hiredCandidates?.length !== 0 ? (
            <tbody>
              {candidateList?.hiredCandidates?.map((item: any) => {
                let totalYears = 0;
                let totalMonths = 0;
                item?.user_info?.total_experiences?.forEach(
                  (experience: any) => {
                ...
                return (
                  <tr>
                    ...
                    <td className="px-6 py-4 text-gray-500">
                      <div className="flex items-center">
                        <div className="flex w-full items-center text-base font-medium text-meta-purple-1">
                          {APPLICANT_STATUS?.[0]}
                        </div>
